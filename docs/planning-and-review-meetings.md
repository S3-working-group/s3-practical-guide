...
-   Review the product backlog and select work items for the next iteration (the “iteration [backlog](backlog.html)”) by considering available capacity, estimated effort, priority, and dependencies.
...
-   Share insights on what worked well and where improvements may be required. (Add those to the agenda for a [Retrospective](retrospective.html) meeting, but refrain from attempting to deal with them in the Review meeting, which should maintain focus on reviewing the actual work).
