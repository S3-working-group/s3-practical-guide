# Retrospective

<summary>
Dedicate time to reflect on past experience, learn, and decide how to improve work processes.
</summary>

-   output: changes to work process, new tasks, on-the-fly [policies](glossary:policy), and  new [drivers](glossary:organizational-driver) or [requirements](glossary:requirement)
-   facilitated meeting (~1hr)
-   regular intervals (1-4 weeks)
-   adapt to situation and context

![Output of a retrospective](img/meetings/retrospective.png#limit=width;scale=small)

## Five Phases of a Retrospective Meeting

1. Set the stage
2. Gather data
3. Generate insights
4. Decide what to do
5. Close the retrospective

Many different activities for each phase can be found at [plans-for-retrospectives.com](http://www.plans-for-retrospectives.com/)
