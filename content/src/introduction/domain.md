...
[Clarifying domains](section:clarify-domains) makes the contract between [delegator](glossary:delegator) and [delegatee](glossary:delegatee) explicit, which enables everyone to learn about what works and what doesn't, and to understand who is responsible for what. A clear domain description with a reasonable amount of detail is a necessary prerequisite for people to successfully evaluate and continuously improve their work.
...
Responsibility for domains is delegated to people (e.g. to a unit, department, team or individuals), who then act within its defined constraints on their autonomy and influence.

When a domain is delegated to a group of people, they become a _team_, when it's delegated to an individual, they become a _role keeper_.
...
**Note:** _Things that are forbidden include explicit constraints laid out in the domain description, any other agreements the delegatee needs to keep, and legal and regulatory requirements._

Those delegating responsibility for a domain (the delegator) to others still retain overall accountability for that domain, allocate **resources** and often define:
