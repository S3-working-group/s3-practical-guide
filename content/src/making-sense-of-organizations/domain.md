...
[Clarifying domains](section:clarify-and-develop-domains) makes the contract between the *delegator* (who delegates responsibility for a domain) and the *delegatee(s)* (to whom the domain is delegated) explicit, which enables everyone to understand expectations and to reflect on what works and what doesn't, so that a domain's design can be improved over time. A clear domain description with a reasonable amount of detail is a necessary prerequisite for people to successfully evaluate and continuously improve their work.
...
When a domain is delegated to a group of people, they form a [team](glossary:team). When a domain is delegated to an individual, they become a [role keeper](glossary:role-keeper).
...
See [Clarify and Develop Domains](section:clarify-and-develop-domains) for more details.
