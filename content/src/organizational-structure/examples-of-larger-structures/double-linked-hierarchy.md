# Double-Linked Hierarchy

Delegate all authority for making governance decisions to circles, double-linked across all levels of the hierarchy, to transition from a traditional hierarchy towards a structure more suitable for tapping collective intelligence, ensuring equivalence, and building engagement.

1. On all levels of your organization, shift responsibility for governance decision-making from individuals to [circles](section:circle) by granting teams formal  authority for making certain governance decisions themselves.
2. Each circle's members select one of their group to represent their interests and participate in the [governance](glossary:governance) decision-making of the next higher circle, and vice versa.

A double-linked hierarchy:

-   brings equivalence to governance
-   maintains the potential for a functional hierarchy (if it enables the flow of value).

![A double-linked hierarchy: not your typical hierarchy](img/structural-patterns/double-linked-hierarchy.png#limit=height;scale=small)

**See also**: [Circle](section:circle), [Double Linking](section:double-linking), [Representative](section:representative)
